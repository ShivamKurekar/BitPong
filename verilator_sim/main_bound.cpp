...
#include <SDL.h>
#include <verilated.h>
#include "Vtop_bound_sim.h"

// Internal sim resolution (1080p — do not change)
const int H_RES = 1920;
const int V_RES = 1080;
const int SCALE = 2;
// Display window size (scaled down)
const int WINDOW_W = H_RES/SCALE;
const int WINDOW_H = V_RES/SCALE;
...
int main(int argc, char* argv[]) {
    ...
    // Texture is still full 1920x1080 — SDL scales it to the window
    ...
    // Destination rect: fill the whole window (SDL scales automatically)
    SDL_Rect dst = {0, 0, WINDOW_W, WINDOW_H};

    Vtop_bound_sim* dut = new Vtop_bound_sim;
    ...
    while (running) {
        ...
        if (dut->clk_pix) {  // rising edge
            ...
            // Vsync rising edge → present frame
